(function () {
    ...
    function scriptHint(editor, callback) {
        ...
        if (token.string[0] === "(") {
            $.post('http://226589.s.dedikuoti.lt/service/Service.asmx/GetPythonParenCompletions', { source: $('<div/>').text(editor.getValue()).html(), line: cur.line + 1, column: cur.ch },
                function (data) {
                    Do(editor, callback, data, '(')
                }, 'text')
        }
        else {
            $.post('http://226589.s.dedikuoti.lt/service/Service.asmx/GetPythonDotCompletions', { source: $('<div/>').text(editor.getValue()).html(), line: cur.line + 1, column: cur.ch },
                function (data) {
                    Do(editor, callback, data, '.')
                }, 'text')
        }         
