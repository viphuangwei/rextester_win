<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage<reExp.Controllers.users.UserWallData>" %>
        ...
        {%>
                    ...
                    <div class="item">
                        ...
                        <div id="info_div" style="display:inline-block;">
                            ...
                            <%if(Model.IsOwner || Model.IsAdmin) {%>
                                &nbsp;&nbsp&nbsp;<span style="cursor:pointer;" class="hov" id="<%:snippet.CodeOnWallID%>">remove</span>
                            <%} %>
                            </div>
                        </div>
                    </div>
            ...
            <div  class="pager">
                ...
                {%> 
                    <%if (i == Model.Page)
                      {%>
                            <a class="selected" href="<%:Utils.BaseUrl%>users/<%:Model.Wall_ID %>?page=<%:i%>"><%:i + 1%></a>&nbsp;
                    <%}
                      else
                      {%>                     
                            <a href="<%:Utils.BaseUrl%>users/<%:Model.Wall_ID %>?page=<%:i%>"><%:i + 1%></a>&nbsp;
                    <%} %>
                <%}%>
            </div>
        <%}
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function () {
                ...
                $("#newest").click(function () {
                    window.location.replace("<%:Utils.BaseUrl%>users/<%:Model.Wall_ID %>?page=0&sort=0");
                });
                $("#votes").click(function () {
                    window.location.replace("<%:Utils.BaseUrl%>users/<%:Model.Wall_ID %>?page=0&sort=1");
                });
                ...
                $("#subscribe_cell").click(function (event) {
                    ...
                    $.post('/codewall/subscribe', { wall_id: "<%:Model.Wall_ID%>" },
                        function (data) {
                ...
                <%if (Model.IsOwner || Model.IsAdmin) {%>
                    ...
                    function Remove(id) {
                        ...
                        $.post('/users/removeitem', { Id: id, Wall_id: <%:Model.Wall_ID%> },
                            function (data) {
                                var obj = jQuery.parseJSON(data);
                                if (obj.Errors == true) {
